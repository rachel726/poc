...
public class pocMenuContributor : IMenuContributor
{
    ...
    private static Task ConfigureMainMenuAsync(MenuConfigurationContext context)
    {
        ...
        context.Menu.AddItem(
            new ApplicationMenuItem(
                pocMenus.HostDashboard,
                l["Menu:Dashboard"],
                "~/HostDashboard",
                icon: "fa fa-line-chart",
                order: 2
            ).RequirePermissions(pocPermissions.Dashboard.Host)
        );

        //TenantDashboard
        context.Menu.AddItem(
            new ApplicationMenuItem(
                pocMenus.TenantDashboard,
                l["Menu:Dashboard"],
                "~/Dashboard",
                icon: "fa fa-line-chart",
                order: 2
            ).RequirePermissions(pocPermissions.Dashboard.Tenant)
        );
